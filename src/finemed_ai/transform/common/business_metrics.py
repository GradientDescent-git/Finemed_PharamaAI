...
from finemed_ai.transform.common.helper_functions import safe_divide
...
def calculate_profit_margin(revenue: float,cost: float) -> float:
    ...
    return round(safe_divide(profit * 100,revenue),2)

def calculate_percentage_change(current_value: float,previous_value: float) -> float:
    difference = current_value - previous_value
    return round(safe_divide(difference * 100,previous_value),2)
...
def calculate_inventory_turnover(cost_of_goods_sold: float,average_inventory: float) -> float:
    return round(safe_divide(cost_of_goods_sold,average_inventory),2)

def calculate_days_of_inventory(inventory_turnover: float) -> float:
    return round(safe_divide(365,inventory_turnover,),2)

def calculate_stock_availability(inventory_df: pd.DataFrame,stock_column: str,) -> float:
    validate_columns_exist(inventory_df,[stock_column])
    return float(inventory_df[stock_column].sum())

def calculate_inventory_value(inventory_df: pd.DataFrame,quantity_column: str,unit_cost_column: str) -> float:
    validate_columns_exist(inventory_df,[quantity_column,unit_cost_column])
    return float((inventory_df[quantity_column] * inventory_df[unit_cost_column]).sum())

    return float((inventory_df[quantity_column] * inventory_df[unit_cost_column]).sum())

def calculate_stock_utilization(sold_quantity: float,purchased_quantity: float) -> float:
    return round(safe_divide(sold_quantity * 100,purchased_quantity),2)

def calculate_stock_out_rate(stock_out_count: int,total_orders: int) -> float:
    return round(safe_divide(stock_out_count * 100,total_orders),2)
